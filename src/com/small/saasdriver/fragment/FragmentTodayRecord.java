...
public class FragmentTodayRecord extends BaseFragment {
	...
	protected View initView() {
		View view = View.inflate(mActivity, R.layout.fragment_today_record, null);
		lv_record = (ListView) view.findViewById(R.id.lv_today_record);
		String resJson;
		try {
	...
	private void sendDataToNet() {// 请求网络
		...
		params.addBodyParameter("DayOrAll", "1");
		...
		utils.send(HttpMethod.POST, HttpConstant.ORDER_DATA_RECORD, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {// 请求成功
				...
				try {
					CacheUtil.setFileCache(mActivity, HttpConstant.ORDER_DATA_RECORD, result);// 缓存
				} catch (Exception e) {
					e.printStackTrace();
				}
