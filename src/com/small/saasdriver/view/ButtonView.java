...
public class ButtonView extends View implements View.OnClickListener {
	...
	private int textColor = R.color.arcColor;
	private int strokeSize = 2;
	private int strokeColor = R.color.papayawhip;
	...
	public ButtonView(Context context, AttributeSet attrs) {
		...
		textSize = a.getDimensionPixelSize(R.styleable.ButtonView_text_size,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
						textSize, dm));
		strokeSize = a.getDimensionPixelSize(
				R.styleable.ButtonView_stroke_size, (int) TypedValue
						.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
								strokeSize, dm));
		textColor = a.getColor(R.styleable.ButtonView_text_color,
				getResources().getColor(textColor));
		strokeColor = a.getColor(R.styleable.ButtonView_stroke_color,
				getResources().getColor(strokeColor));
		arcColor = a.getColor(R.styleable.ButtonView_arc_color, getResources()
				.getColor(arcColor));
		a.recycle();

		paddingOffset = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, paddingOffset, dm);
		...
		if (drawWidth != smallF) {
			if (drawWidth >= bigF) {
				canvas.drawText(
						...
						height
								/ 2
								+ Math.abs((textPaint.descent() + textPaint
										.ascent()) / 2), textPaint);
			}
		} else {
	...
	private RectF getRectF() {
		return new RectF(width - drawWidth + getPaddingLeft() + paddingOffset,
				getPaddingTop() + paddingOffset, drawWidth - getPaddingRight()
						- paddingOffset, height - getPaddingBottom()
						- paddingOffset);
	}
