...
import com.saas.chat.bean.PublicMsgInfo;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(
					R.layout.em_layout_item_msg, parent, false);
		}
		...
		if (PublicMsg != null && position == 0) {
			...
			img_avar.setImageResource(R.drawable.em_icon_public);
			...
			if (conversation.isGroup()) {
				img_avar.setImageResource(R.drawable.em_defult_group);
			...
			if (conversation.getMsgCount() != 0) {
				...
				if (lastMessage.status == EMMessage.Status.SUCCESS) {
					...
					txt_state.setBackgroundResource(R.drawable.em_btn_bg_blue);
				}
			}
