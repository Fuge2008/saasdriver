...
public class GroupSettingActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		setContentView(R.layout.em_activity_groupsetting);
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = LayoutInflater.from(context).inflate(
						R.layout.em_item_chatsetting_gridview, null);
			}
			...
			if (position == getCount() - 1 && is_admin) {
				...
				iv_avatar.setImageResource(R.drawable.em_icon_btn_deleteperson);
				...
				tv_username.setText("");
				badge_delete.setVisibility(View.GONE);
				iv_avatar.setImageResource(R.drawable.em_jy_drltsz_btn_addperson);
				// 正处于删除模式下,隐藏添加按钮
				if (isInDeleteMode) {
					convertView.setVisibility(View.GONE);
				} else {
				...
				iv_avatar.setImageResource(R.drawable.em_head);// TODO 网络加载头像
